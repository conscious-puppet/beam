...
module Database.Beam.Postgres.ConnectionP
  ( Pg(..), PgF(..)
...
postgresUriSyntax :: c PostgresP Pg.Connection Pg
                  -> BeamURIOpeners c
postgresUriSyntax =
...
runPgRowReader ::
  Pg.Connection -> Pg.Row -> Pg.Result -> [Pg.Field] -> FromBackendRowM PostgresP a -> IO (Either BeamRowReadError a)
runPgRowReader conn rowIdx res fields (FromBackendRowM readRow) =
  ...
  where
    ...
    step :: forall x. FromBackendRowF PostgresP (CInt -> CInt -> [PgI.Field] -> IO (Either BeamRowReadError x))
...
withPgDebug dbg conn (Pg action) =
  let finish x = pure (Right x)
      ...
      step (PgRunReturning CursorBatching command (mkProcess :: Pg (Maybe x) -> Pg a') next) = error ("PgRunReturning CursorBatching command: " <> show command)
...
--
--                    columnCount = fromIntegral $ valuesNeeded (Proxy @PostgresP) (Proxy @x)
--                in Pg.foldWith_ (Pg.RP (put columnCount >> ask)) conn (Pg.Query query) (PgStreamContinue nextStream) runConsumer >>= finishUp
      step (PgRunReturning AtOnce command (mkProcess :: Pg (Maybe x) -> Pg a') next) = error ("PgRunReturning AtOnce command"  <> show command)
...
data PgF next where
    ...
    PgRunReturning ::
        FromBackendRow PostgresP x =>
        FetchMode -> Command -> (Pg (Maybe x) -> Pg a) -> (a -> next) -> PgF next
    PgFetchNext ::
        FromBackendRow PostgresP x =>
...
instance MonadBeam PostgresP Pg where
...
-- instance MonadBeamInsertReturning PostgresP Pg where
--     runInsertReturningList i = do
--         let insertReturningCmd' = i `returning`
--               changeBeamRep (\(Columnar' (QExpr s) :: Columnar' (QExpr PostgresP PostgresInaccessible) ty) ->
--                 Columnar' (QExpr s) :: Columnar' (QExpr PostgresP ()) ty)
--
--         -- Make savepoint
...
--
-- instance MonadBeamUpdateReturning PostgresP Pg where
...
--
-- instance MonadBeamDeleteReturning PostgresP Pg where
