...
module Database.Beam.Postgres.FullP
...
import           Database.Beam.Postgres.TypesP
...
locked_ :: (Beamable tbl, Database PostgresP db)
        => DatabaseEntity PostgresP db (TableEntity tbl)
        -> Q PostgresP db s (PgLockedTables s, tbl (QExpr PostgresP s))
...
lockingFor_ :: forall a db s
             . ( Database PostgresP db, Projectible PostgresP a, ThreadRewritable (QNested s) a )
            => PgSelectLockingStrength
            -> Maybe PgSelectLockingOptions
            -> Q PostgresP db (QNested s) (PgWithLocking (QNested s) a)
            -> Q PostgresP db s (WithRewrittenThread (QNested s) s a)
lockingFor_ lockStrength mLockOptions (Q q) = undefined
...
-- empty @FOR .. OF@ clause.
lockingAllTablesFor_ :: ( Database PostgresP db, Projectible PostgresP a, ThreadRewritable (QNested s) a )
                     => PgSelectLockingStrength
                     -> Maybe PgSelectLockingOptions
                     -> Q PostgresP db (QNested s) a
                     -> Q PostgresP db s (WithRewrittenThread (QNested s) s a)
lockingAllTablesFor_ lockStrength mLockOptions q = undefined
...
insertDefaults :: SqlInsertValues PostgresP tbl
...
-- see 'insertReturning'.
insert :: DatabaseEntity PostgresP db (TableEntity table)
       -> SqlInsertValues PostgresP (table (QExpr PostgresP s)) -- TODO arbitrary projectibles
       -> PgInsertOnConflict table
       -> SqlInsert PostgresP table
insert tbl@(DatabaseEntity dt@(DatabaseTable {})) values onConflict_ = undefined
-- insert tbl@(DatabaseEntity dt@(DatabaseTable {})) values onConflict_ =
--   case insertReturning tbl values onConflict_
--          (Nothing :: Maybe (table (QExpr PostgresP PostgresInaccessible) -> QExpr PostgresP PostgresInaccessible Int)) of
--     PgInsertReturning a ->
...
insertReturning :: Projectible PostgresP a
                => DatabaseEntity PostgresP be (TableEntity table)
                -> SqlInsertValues PostgresP (table (QExpr PostgresP s))
                -> PgInsertOnConflict table
                -> Maybe (table (QExpr PostgresP PostgresInaccessible) -> a)
...
--      Just mkProjection ->
--          emit " RETURNING " <>
--          pgSepBy (emit ", ") (map fromPgExpression (project (Proxy @PostgresP) (mkProjection tblQ) "t")))
--    where
...
runPgInsertReturningList
  :: ( MonadBeam be m
     , BeamSqlBackendSyntax be ~ Command
     , FromBackendRow be a
     )
  => PgInsertReturning a
  -> m [a]
runPgInsertReturningList = undefined
...
lateral_ :: forall s a b db
          . ( ThreadRewritable s a, ThreadRewritable (QNested s) b, Projectible PostgresP b )
         => a -> (WithRewrittenThread s (QNested s) a -> Q PostgresP db (QNested s) b)
         -> Q PostgresP db s (WithRewrittenThread (QNested s) s b)
lateral_ using mkSubquery = undefined
...
onConflict :: Beamable tbl
           => SqlConflictTarget PostgresP tbl
           -> SqlConflictAction PostgresP tbl
...
conflictingConstraint :: T.Text -> SqlConflictTarget PostgresP tbl
conflictingConstraint nm = undefined
...
updateReturning :: Projectible PostgresP a
                => DatabaseEntity PostgresP be (TableEntity table)
                -> (forall s. table (QField s) -> QAssignment PostgresP s)
                -> (forall s. table (QExpr PostgresP s) -> QExpr PostgresP s Bool)
                -> (table (QExpr PostgresP PostgresInaccessible) -> a)
...
runPgUpdateReturningList
  ...
  => PgUpdateReturning a
  -> m [a]
runPgUpdateReturningList = undefined
...
deleteReturning :: Projectible PostgresP a
                => DatabaseEntity PostgresP be (TableEntity table)
                -> (forall s. table (QExpr PostgresP s) -> QExpr PostgresP s Bool)
                -> (table (QExpr PostgresP PostgresInaccessible) -> a)
...
runPgDeleteReturningList = undefined
...
class PgReturning cmd where
  ...
  returning :: (Beamable tbl, Projectible PostgresP a)
            => cmd PostgresP tbl -> (tbl (QExpr PostgresP PostgresInaccessible) -> a)
...
-- instance BeamHasInsertOnConflict PostgresP where
--   newtype SqlConflictTarget PostgresP table =
--     PgInsertOnConflictTarget (table (QExpr PostgresP QInternal) -> PgInsertOnConflictTargetSyntax)
--   newtype SqlConflictAction PostgresP table =
...
--               project (Proxy @PostgresP) (makeProjection tbl) "t") <>
--     emit " "
--
...
--     PgInsertOnConflictTargetSyntax $
--     pgParens (pgSepBy (emit ", ") $
--               map fromPgExpression (project (Proxy @PostgresP)
